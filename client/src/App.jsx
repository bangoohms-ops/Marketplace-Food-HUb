...
function App() {
  ...
  const sampleData = [
    ...
    },
    {
      ...
      price: 4500,
      image_url:
        "https://images.unsplash.com/photo-1628102422204-706cc6e3c0b1?w=800",
      category: "Swallow",
    },
    {
      id: 3,
    ...
    {
      id: 8,
      name: "Golden Fried Yam & Eggs",
      price: 3800,
      image_url:
        "https://images.unsplash.com/photo-1599307767316-776533da941c?w=800",
      category: "Sides",
    },
                    ...
                    {getItemQty(product.id) === 0 ? (
                      ...
                      <div className="flex items-center justify-between bg-gray-100 rounded-2xl p-2 border-2 border-black">
                        <button
                          ...
                          className="w-12 h-12 flex items-center justify-center bg-white rounded-xl font-black text-2xl hover:text-orange-600 transition"
                        >
                          -
                        </button>
      ...
      {view === "login" && (
        <div className="h-screen bg-[#0a0a0a] flex items-center justify-center p-6">
          <div className="w-full max-w-md bg-[#141414] border border-white/5 p-10 rounded-[3rem] shadow-2xl text-center">
            <h2 className="text-white text-3xl font-black italic uppercase tracking-tighter mb-8">
              Fresh Foods Auth
            </h2>
            <form onSubmit={handleLogin} className="space-y-6">
              <input
                ...
                autoFocus
              />
              <button className="w-full bg-white text-black font-black py-5 rounded-3xl uppercase tracking-widest hover:bg-orange-500 transition-all">
                Verify Identity
              </button>
              <button
                type="button"
                onClick={() => setView("menu")}
                className="text-gray-600 text-xs font-bold uppercase tracking-widest block w-full mt-4"
              >
                Cancel
              </button>
            </form>
      ...
      {view === "admin" && (
        <div className="flex flex-col min-h-screen bg-[#0f172a]">
          <nav className="bg-black border-b border-white/10 px-6 py-4 flex justify-between items-center sticky top-0 z-[100]">
            <div className="flex gap-2">
              <button
                ...
                className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${adminTab === "pos" ? "bg-orange-600 text-white shadow-lg shadow-orange-900/40" : "text-gray-500 hover:text-white"}`}
              >
                POS System
              </button>
              {userRole === "ceo" && (
                <button
                  ...
                  className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${adminTab === "dashboard" ? "bg-orange-600 text-white shadow-lg shadow-orange-900/40" : "text-gray-500 hover:text-white"}`}
                >
                  CEO Dashboard
                </button>
              )}
            ...
            <div className="flex items-center gap-4">
              <span className="hidden md:block text-[9px] font-black text-orange-500 uppercase border border-orange-500/20 px-3 py-1 rounded-full">
                Session: {userRole}
              </span>
              <button
                onClick={() => {
                ...
                className="text-[10px] font-black text-gray-500 hover:text-red-500 uppercase transition-colors"
              >
                Logout
              </button>
            </div>
          ...
          <div className="flex-1">
            {adminTab === "pos" ? <POS_Screen /> : <AdminDashboard />}
          </div>
        </div>
      )}
    </div>
