...
function App() {
  ...
  // 3. EFFECTS & FETCHING
  useEffect(() => {
    // Falls back to your local port 5001 server baseline cleanly
        ...
        <>
          <header className="fixed top-0 left-0 right-0 h-20 bg-white/90 backdrop-blur-xl z-[100] border-b border-gray-100 px-4 md:px-10 flex justify-between items-center">
            ...
            <div className="flex gap-4 md:gap-6 items-center">
              <button onClick={() => setView('login')} className="text-gray-400 text-[9px] font-bold uppercase tracking-widest hover:text-black transition-colors">Staff Portal</button>
              <button onClick={() => setView('checkout')} className="bg-black text-white px-6 md:px-8 py-3 rounded-full font-bold text-xs md:text-sm uppercase tracking-widest hover:scale-105 transition shadow-lg">Cart ({totalItems})</button>
            </div>
          </header>
          ...
          <main className="pt-32 pb-16 px-6 md:px-10 max-w-[1400px] mx-auto">
            <div className="mb-16">
              ...
              <h2 className="text-5xl md:text-8xl font-black tracking-tighter italic leading-none">Our <span className="text-orange-600">Menu.</span></h2>
            </div>
                ...
                <div key={product.id} className="group">
                  ...
                  <div className="mt-8">
                    {getItemQty(product.id) === 0 ? (
                      <button onClick={() => addToCart(product)} className="w-full py-5 bg-black text-white rounded-2xl font-bold uppercase tracking-widest hover:bg-orange-600 transition-colors shadow-lg active:scale-95">Add To Cart +</button>
                    ) : (
        ...
        <div className="h-screen bg-[#0a0a0a] flex items-center justify-center p-6">
          <div className="w-full max-w-md bg-[#141414] border border-white/5 p-10 rounded-[3rem] shadow-2xl text-center">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <input type="password" value={pinInput} onChange={(e) => setPinInput(e.target.value)} placeholder="ENTER PIN" className="w-full bg-black border border-white/10 text-white text-center text-4xl py-6 rounded-3xl focus:border-orange-500 outline-none transition-all tracking-[0.5em]" autoFocus />
              <button type="submit" className="w-full bg-white text-black font-black py-5 rounded-3xl uppercase tracking-widest hover:bg-orange-500 transition-all">Verify Identity</button>
