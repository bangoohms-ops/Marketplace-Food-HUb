const express = require('express');
const { Pool } = require('pg');
const cors = require('cors');
const nodemailer = require('nodemailer');
require('dotenv').config();

const app = express();
const PORT = process.env.PORT || 5001;

// --- 1. MIDDLEWARE ---
app.use(cors());
app.use(express.json());

// --- 2. POSTGRES CONNECTION ---
// Added a check to ensure DATABASE_URL exists to prevent crash on startup
if (!process.env.DATABASE_URL) {
  console.error("❌ ERROR: DATABASE_URL is missing in .env");
  process.exit(1);
}

const pool = new Pool({
  connectionString: process.env.DATABASE_URL,
  ssl: { rejectUnauthorized: false }
});

// --- 3. EMAIL CONFIGURATION ---
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: process.env.EMAIL_USER,
    pass: process.env.EMAIL_PASS // Reminder: Use an "App Password," not your main password
  }
});

// Verify email connection on start
transporter.verify((error, success) => {
  if (error) console.log("📧 Email Server Error:", error.message);
  else console.log("📧 Email Server is ready");
});

// --- 4. DATABASE INITIALIZATION ---
const initializeDatabase = async () => {
  const createTablesQuery = `
    CREATE TABLE IF NOT EXISTS products (
      id SERIAL PRIMARY KEY,
      name VARCHAR(255) NOT NULL,
      price DECIMAL(12, 2) NOT NULL,
      category VARCHAR(100),
      image_url TEXT
    );

    CREATE TABLE IF NOT EXISTS sales (
      id SERIAL PRIMARY KEY,
      items JSONB NOT NULL,
      total_price DECIMAL(12, 2) NOT NULL,
      payment_method VARCHAR(50) DEFAULT 'Cash',
      staff_name VARCHAR(100) DEFAULT 'Admin',
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );

    CREATE TABLE IF NOT EXISTS orders (
      id SERIAL PRIMARY KEY,
      full_name TEXT NOT NULL,
      phone TEXT NOT NULL,
      email TEXT,
      address TEXT NOT NULL,
      grand_total DECIMAL(12, 2) NOT NULL,
      payment_status TEXT DEFAULT 'Pending',
      items JSONB NOT NULL,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
  `;

  try {
    const client = await pool.connect();
    await client.query(createTablesQuery);
    console.log("✅ PostgreSQL Tables Verified");
    client.release();
  } catch (err) {
    console.error("❌ DB Initialization Error:", err.message);
  }
};

initializeDatabase();

// --- 5. API ROUTES ---

app.get('/', (req, res) => res.send('Bango Food Hub Backend is Running...'));

// --- SALES ROUTES ---
app.get('/api/sales', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM sales ORDER BY created_at DESC');
    res.json(result.rows);
  } catch (err) {
    console.error("❌ Fetch Sales Error:", err.message);
    res.status(500).json({ error: "Could not fetch sales data" });
  }
});

app.post('/api/sales', async (req, res) => {
  const { items, total_price, payment_method, staff_name } = req.body;
  
  // Basic validation
  if (!items || !total_price) {
    return res.status(400).json({ error: "Missing required sales data" });
  }

  try {
    const newSale = await pool.query(
      `INSERT INTO sales (items, total_price, payment_method, staff_name) 
       VALUES ($1, $2, $3, $4) RETURNING *`,
      [JSON.stringify(items), total_price, payment_method, staff_name]
    );
    res.status(201).json(newSale.rows[0]);
  } catch (err) {
    console.error("❌ Sale Error:", err.message);
    res.status(500).json({ error: "Could not record sale" });
  }
});

// --- ORDER ROUTES ---
app.post('/api/orders', async (req, res) => {
    try {
      const { fullName, phone, email, address, grandTotal, paymentStatus, items } = req.body;
      
      // Use a transaction if you eventually add inventory management here
      const newOrder = await pool.query(
        `INSERT INTO orders (full_name, phone, email, address, grand_total, payment_status, items) 
         VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
        [fullName, phone, email, address, grandTotal, paymentStatus || 'Pending', JSON.stringify(items)]
      );

      // Email Logic - Wrapped in a try/catch so the order still completes even if email fails
      if (email) {
        try {
            const mailOptions = {
                from: `"Bango Food Hub" <${process.env.EMAIL_USER}>`,
                to: email,
                subject: 'Order Confirmed! 🚀 - Bango Food Hub',
                html: `
                  <div style="font-family: sans-serif; max-width: 600px; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
                    <h1 style="color: #ea580c;">Bango Food Hub</h1>
                    <h2>Order Received, ${fullName}!</h2>
                    <p><strong>Total Paid:</strong> ₦${Number(grandTotal).toLocaleString()}</p>
                    <p><strong>Delivery Address:</strong> ${address}</p>
                    <hr style="border: 0; border-top: 1px solid #eee;" />
                    <p>Our team is preparing your order. Stay tuned!</p>
                  </div>
                `
              };
              await transporter.sendMail(mailOptions);
        } catch (mailErr) {
            console.error("📧 Mail delivery failed:", mailErr.message);
        }
      }
  
      res.status(201).json(newOrder.rows[0]);
    } catch (err) {
      console.error("Order Error:", err.message);
      res.status(500).json({ error: "Order failed" });
    }
});

// --- PRODUCT ROUTES ---
app.get('/api/products', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM products ORDER BY id DESC');
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

app.listen(PORT, () => {
  console.log(`🚀 Server spinning on port ${PORT}`);
});